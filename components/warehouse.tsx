  ...
  return (
    <div className="tablet-container">
      ...
      {mode === "home" ? (
        ...
        <div className="success-view"><div className="success-icon"><Check size={36}/></div><h1>Saída registada.</h1><p>O stock, os equipamentos e o custo da obra foram atualizados.</p><div className="success-summary"><b>{mixedLines.length} recurso{mixedLines.length === 1 ? "" : "s"} registado{mixedLines.length === 1 ? "" : "s"}</b><span>Obra {work} · {state.works.find(w => w.id === work)?.nome}</span><span>{date(effective)} · {money(mixedValue)} no primeiro dia</span></div><div className="header-actions"><Button onClick={() => reset("register")}>Registar outra saída</Button><Button secondary onClick={() => reset("home")}>Voltar ao início</Button></div></div>
      ) : mode === "register" ? (
        <div className="tablet-flow">
          {step === 0 ? <>
            <div className="tablet-greeting"><div className="eyebrow">PASSO 1</div><h1>Selecionar obra</h1><p>Todo o custo ou utilização fica ligado a esta obra.</p></div>
            ...
            <div className="tablet-home-grid">
              <button className="primary" onClick={() => {setResourceKind("article");setStep(2);}}><Package size={30}/><div><h2>Consumível ou material</h2><p>Quantidade × valor interno</p></div><ArrowRight size={21}/></button>
              <button onClick={() => {setResourceKind("machine");setStep(2);}}><Wrench size={30}/><div><h2>Maquinaria ou equipamento</h2><p>Alocação individual × custo diário</p></div><ArrowRight size={21}/></button>
            </div>
            ...
            {resourceKind === "machine" ? <Note>Este recurso é identificado individualmente. A quantidade da saída é 1 e o custo é calculado por dia até à devolução.</Note> : special ? <Note tone="amber">Cada bobine retirada é imputada integralmente na primeira saída.</Note> : null}
            ...
            <p className="quantity-help">{resourceKind === "article" ? `${qty(available,article)} disponíveis` : `${money(selectedMachine?.custoDia ?? 0)} por dia`}</p>
            ...
            <div className="movement-list">{mixedLines.map((line,index)=>{const a=line.kind === "article" ? state.articles.find(item=>item.id===line.id) : undefined;const m=line.kind === "machine" ? state.machines.find(item=>item.id===line.id) : undefined;return <div key={`${line.kind}-${line.id}-${index}`}><span>{line.kind === "article" ? `${a?.codigoACRS} · ${a?.descricao}` : `${m?.numero} · ${m?.nome}`}</span><b>{line.kind === "article" ? `${qty(line.quantity,a)} · ${money(line.quantity*line.rate)}` : `1 equipamento · ${money(line.rate)} / dia`}</b><button className="text-button" onClick={()=>{const next=mixedLines.filter((_,i)=>i!==index);setMixedLines(next);if(!next.length)setStep(1);}}>Remover</button></div>;})}</div>
            <DetailList items={[["Obra",`${work} · ${state.works.find(w=>w.id===work)?.nome}`],["Recursos",String(mixedLines.length)],["Custo inicial",money(mixedValue)],["Data efetiva",date(effective)],["Registado por",profile]]}/><Note>Consumíveis reduzem o stock. Máquinas e equipamentos ficam alocados à obra e acumulam custo diário até à devolução.</Note>{error && <Note tone="red">{error}</Note>}<div className="form-actions"><Button secondary onClick={()=>setStep(5)}>Alterar data</Button><Button onClick={confirmMixed}>Confirmar saída <Check size={17}/></Button></div>
          </>}
        </div>
      ) : mode === "stock" ? (
        <Resources />
