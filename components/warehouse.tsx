...
export function Tablet() {
  ...
  const hideFinancials = profile === "Armazém";
      ...
      {mode === "home" ? (
        ...
        <div className="tablet-flow">
          {step === 0 ? <>
            ...
            <div className="tablet-home-grid">
              <button className="primary" onClick={() => {setResourceKind("article");setStep(2);}}><Package size={30}/><div><h2>Consumível ou material</h2><p>Selecionar quantidade</p></div><ArrowRight size={21}/></button>
            ...
            {resourceKind === "machine" ? <Note>Este recurso é identificado individualmente. A quantidade da saída é 1 e fica associado à obra até à devolução.</Note> : special ? <Note tone="amber">Cada bobine retirada é imputada integralmente na primeira saída.</Note> : null}
          </> : step === 3 ? <>
            ...
            <p className="quantity-help">{resourceKind === "article" ? `${qty(available,article)} disponíveis` : "1 unidade disponível"}</p>
            ...
            <DetailList items={[["Obra",`${work} · ${state.works.find(w=>w.id===work)?.nome}`],["Recursos",String(mixedLines.length)],["Data efetiva",date(effective)],["Registado por",profile],...(!hideFinancials ? [["Custo inicial",money(mixedValue)] as [string,string]] : [])]}/><Note>Consumíveis reduzem o stock. Máquinas e equipamentos ficam associados à obra até à devolução.</Note>{error && <Note tone="red">{error}</Note>}<div className="form-actions"><Button secondary onClick={()=>setStep(5)}>Alterar data</Button><Button onClick={confirmMixed}>Confirmar saída <Check size={17}/></Button></div>
          </>}
        </div>
      ) : mode === "stock" ? (
        <Resources hideFinancials={hideFinancials} />
      ) : mode === "machines" ? (
